...
export default function CourseDetail() {
  ...
  return (
    <div>
      <PageHead title={
        <span>{course.name}
          <div className="page-sub">{course.subject} · {course.class_id_label} · {course.semester}</div>
        </span>}>
        <button className="btn secondary" onClick={() => nav('/courses')}>← Back</button>
        <button className="btn secondary" onClick={() => setEditing(true)}>Edit Course</button>
        <DeleteButton entity="courses" id={course.id} onDeleted={() => nav('/courses')} />
      </PageHead>

      <Tabs tabs={[
        { key: 'overview', label: 'Overview', render: overview },
        { key: 'pupils', label: 'Enrolled Pupils', count: c.pupils?.length || 0, render: pupilsTab },
        {
          key: 'lessons', label: 'Lessons Given', count: c.lessons?.length || 0,
          render: () => <ChildTable title="Lessons Given" rows={c.lessons} columns={[
        ...
        {
          key: 'exams', label: 'Exams', count: c.exams?.length || 0,
          render: () => <ChildTable title="Exams" rows={c.exams} columns={[
            { key: 'name', label: 'Exam' },
